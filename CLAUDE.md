...
**Still "No settings file found" even via F5 — open `BadgeTracker.code-workspace`, not the bare folder.** The extension names its build output after the VS Code *workspace folder's name*, not the app: with a plain "Open Folder" on this repo (named `garminbadges-watch`), it built `bin/garminbadgeswatch.prg` + a correctly-paired `bin/garminbadgeswatch-settings.json` — but the simulator installs the app under its manifest/properties-derived name, `BADGETRACKER.PRG`, and only pushes the settings JSON under the *source* basename (`GARMINBADGESWATCH-settings.json`), so the two mismatch and the simulator can't find settings for the installed `BADGETRACKER` app. `BadgeTracker.code-workspace` (repo root) pins the workspace folder's display name to `BadgeTracker` via its `"folders": [{ "path": ".", "name": "BadgeTracker" }]` entry, without renaming the actual directory, so the extension's build output basename matches and the settings file lands under the right name. Always open this repo via **File → Open Workspace from File… → `BadgeTracker.code-workspace`**, not File → Open Folder, when you'll need simulator settings editing. Also carries `monkeyC.jungleFiles: "jungle.xml"` (the extension defaults to looking for `monkey.jungle`, which this project doesn't use — see `jungle.xml`).
...
- **`GarminBadgesAllChallengesView`** ("CHALLENGES") — lists all of `_challenges` (up to 20, the API's full `challenges` array, most-urgent-first), using the same full-row layout as Due Soon. `challengeAt(y)`/`showChallengeDetail()` as above.
